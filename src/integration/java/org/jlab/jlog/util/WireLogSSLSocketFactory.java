...
/**
 * This is a utility class useful for debugging. You can wrap an SSLSocketFactory to enable logging
 * of the messages written to the peer. This class is not for use in production and is known to
 * cause Socket connection issues (but usually after it has logged a fair amount of data such as
 * HTTP headers).
 *
 * @author ryans
 */
...
class LoggingOutputStream extends FilterOutputStream {
  private static final Logger logger = Logger.getLogger(LoggingOutputStream.class.getName());
  ...
  public void write(byte[] b, int off, int len) throws IOException {
    if (len > MAX_LOG_MESSAGE_BYTES) {
      logger.log(
          level,
          "write(byte[], int, int) [truncated]: {0}",
          new String(b, off, MAX_LOG_MESSAGE_BYTES, ENCODING));
    } else {
