...
public final class SecurityUtil {
  ...
  /**
   * Obtain a SSLSocketFactory that provides a client certificate from a PEM file and optionally
   * verifies the server's certificate.
   *
   * @param pemPath Path to the PEM file
   * @param verifyPeer true to use the default trust store to verify the server, false to accept the
   *     server's certificate regardless of attributes
   ...
   */
  public static SSLSocketFactory getClientCertSocketFactoryPEM(String pemPath, boolean verifyPeer)
   ...
   * @throws InvalidKeySpecException If unable to create the SocketFactory
   */
  public static SSLContext getContext(String pemPath, boolean verifyPeer)
      throws NoSuchAlgorithmException,
          IOException,
          CertificateException,
          InvalidKeySpecException,
  ...
  /**
   * Parse PEM encoded bytes between type delimiters and return the DER encoded bytes. A PEM file
   * may contain one or more of: a certificate, private key, public key, certificate signing
   * request, certificate signing request response. The items in a PEM file each have a beginning
   * and end delimiter. Example delimiters: <code>
   * -----BEGIN CERTIFICATE-----
   * -----END CERTIFICATE-----
   * </code> PEM encoded bytes are base64 encoded. This method will convert the bytes back into
   * non-base64 encoded binary. This method may need to be called more than once; each time with
   * different delimiters, if the input bytes contain multiple items (i.e. both certificate and
   * private key).
   *
   ...
   */
  public static RSAPrivateKey fetchPrivateKeyFromPEM(byte[] pem)
   ...
   * @return The X509Certificate
   * @throws CertificateException If unable to create a certificate from the bytes
   */
  public static X509Certificate generateX509CertificateFromDER(byte[] certBytes)
      throws CertificateException {
   ...
   */
  public static String getCommonNameFromCertificate(X509Certificate cert)
