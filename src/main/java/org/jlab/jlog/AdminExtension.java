...
/**
 * Wraps a LogItem to provide administrative capabilities. Generally these features will only be
 * needed on the server for administrator use.
 *
 * @author ryans
 */
abstract class AdminExtension {
  ...
  /**
   * Set the author username. Note: the author username is automatically initialized to the
   * submitter (program user). The server will ignore values that do not match the submitter unless
   * the submitter is an administrator.
   *
  ...
  /**
   * Set the created date/time. Note: the created date/time is automatically initialized to the
   * date/time at which the LogEntry object is instantiated.
   *
  ...
  /**
   * Set the log number. This method provides a way to revise a LogEntry without loading its
   * original field values from the server first.
   *
